...
void reduction(int64_t No, int64_t Nv){
  ...
  int64_t scal[] = {};
  std::vector< ivec > s(1);
  CTF::bsTensor<double> V(4, oovv, syms, a, &dw, "V");
  CTF::bsTensor<double> R(4, vvoo, syms, a, &dw, "R");
  CTF::bsTensor<double> S(0, scal, syms, s, &dw, "S"); 
  int64_t el(No*No*Nv*Nv);
  std::vector<double> data(el);
  std::vector<int64_t> idx(el);
  for (int64_t ii(0); ii < el; ii++){
...
//  T.sum(1.0, V, "abij", 1.0, "ijab", false);
  ...
  printf("scalar order: %d\n", S.order); 
  S.contract(1.0, V, "ijab", R, "abij",  0.0, "", true);

  S.read(el, idx.data(), data.data(), 0);
//  for (int64_t ii(0); ii < el; ii++) printf("%f ", data[ii]);
//  printf("=======\n");
//  R.read(el, idx.data(), data.data(), 1);
//  for (int64_t ii(0); ii < el; ii++) printf("%f ", data[ii]);
